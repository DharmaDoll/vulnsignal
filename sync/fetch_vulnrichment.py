...
def _load_rows_from_source_dir(source_dir: Path, source_files: list[Path] | None = None):
...
def sync(
    ...
    try:
        if cache_only:
            ...
            item_iter = _load_rows_from_source_dir(source_dir, source_files=source_files)
            if limit is not None:
