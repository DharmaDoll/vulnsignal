...
|`vex_suppression`  |ranking exclusion if VEX `not_affected` |—  |

## Implementation notes

- All signal lookups must use the **latest** signal per `(vuln_id, signal_type)` ordered by `observed_at DESC`
- VEX suppression is a hard override — a `not_affected` VEX excludes the finding from ranking APIs such as `top_risks()` and `recommend_patch_queue()`
